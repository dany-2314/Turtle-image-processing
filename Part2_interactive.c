...
CmdNode *loadCommands_pro(const char *fileName)
{
  ...
  while (fgets(&buffer[0], 1024, f) != NULL)
  {
    ...
    if (strncmp(buf, "  ", 2) && forN != NULL)
      forN = NULL;
    if (forN != NULL)
      buf = buf + 2; // Skip over indents

    if (!strncmp(buf, "backward", 8))
      temp = newCmdNode("backward", atoi(&buf[9]));
    else if (!strncmp(buf, "forward", 7))
      temp = newCmdNode("forward", atoi(&buf[8]));
    else if (!strncmp(buf, "pendown", 7))
      temp = newCmdNode("pendown", 0);
    else if (!strncmp(buf, "colour", 6))
      temp = newCmdNode("colour", atoi(&buf[7]));
    else if (!strncmp(buf, "penup", 5))
      temp = newCmdNode("penup", 0);
    else if (!strncmp(buf, "right", 5))
      temp = newCmdNode("right", 0);
    else if (!strncmp(buf, "left", 4))
      temp = newCmdNode("left", 0);
    if (!strncmp(buf, "loop", 4))
      forN = newCmdNode("loop", atoi(&buf[5]));

    if (last == NULL)
      head = temp; // If the last node is NULL, then this is the head of the list.

    if (temp != NULL && forN != NULL)
      insertCommandLoop(forN, temp);
    else if (temp != NULL)
      last = insertCommand(last, temp); // Use the last node added to the list as the head.
    else if (forN != NULL)
      last = insertCommand(last, forN); // Use the last node added to the list as the head.

    if (NULL == forN)
      last = temp; // Update the last node only if not inside a loop.
  }
  fclose(f);
  return head;
}
